...
@section('content')
        ...
        <div class="col-lg-12">
            <div class="card animate__animated animate__fadeIn" style="animation-duration: 1s; animation-timing-function: ease-in-out;">
                <div class="card-body">
                    ...
                    <!-- ====================================================== -->
                    <!--     BAGIAN INI YANG DITAMBAHKAN (TOMBOL TAMBAH)      -->
                    <!-- ====================================================== -->
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <h5 class="card-title fw-semibold m-0 animate__animated animate__fadeInLeft" style="animation-duration: 1s; animation-timing-function: ease-in-out;">Daftar Anggota Perpustakaan</h5>
                        
                        <a href="{{ route('member.create') }}" class="btn btn-custom agar warnanya sesuai dengan gradient biru yang Anda inginkan. animate__animated animate__fadeInRight" style="animation-duration: 1s; animation-timing-function: ease-in-out;">
                            ...
                            <tbody class="table-group-divider">
                                @forelse ($members as $index => $member)
                                    <tr class="animate__animated animate__fadeIn" style="animation-duration: 1s; animation-delay: {{ $index * 0.2 }}s; animation-timing-function: ease-in-out;">
                                        <th scope="row">{{ $loop->iteration }}</th> <!-- Diganti agar nomor urut sesuai pagination -->
                                        <td class="text-center">
                                            ...
                                            @else
                                                ...
                                                <img src="{{ asset('path/to/default-avatar.png') }}" alt="Default" class="profile-img">
                                            @endif
                                        </td>
                                        ...
                                        <td class="text-center">
                                            <!-- Status dari Controller (Baru / Lama) -->
                                            <span class="badge status-badge {{ $member->status == 'Baru' ? 'bg-new' : 'bg-other' }} animate__animated animate__pulse" style="animation-duration: 1s; animation-timing-function: ease-in-out;">{{ $member->status }}</span>
                                        </td>
                                        
                                        <!-- ====================================================== -->
                                        <!--        BAGIAN INI YANG DIPERBAIKI (TOMBOL AKSI)        -->
                                        <!-- ====================================================== -->
                                        <td class="text-center">
                                            <div class="d-flex justify-content-center gap-2">
                                                <!-- TOMBOL EDIT/UPDATE -->
                                                <a href="{{ route('member.edit', $member->id) }}" class="btn btn-custom btn-sm">
                                                ...
                                                <form action="{{ route('member.destroy', $member->id) }}" method="POST" style="display: inline;">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-custom btn-sm btn-danger" onclick="return confirmDelete()">
    ...
    <script>
        function confirmDelete() {
            // Ganti confirm() bawaan dengan modal yang lebih cantik jika ada (misal SweetAlert)
    ...
    <style>
        ...
        /* Ganti .status-badge:hover dengan class btn */
        .btn:hover,
        .btn-sm:hover {
            transform: scale(1.05);
        ...
        .bg-other {
            /* 'Lama' (Other) dibuat lebih netral/warning */
        ...
        /* Hapus style duplikat untuk bg-other */
    </style>
